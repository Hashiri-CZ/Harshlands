...
public class SfModule extends HLModule {
    ...
    public SfModule(HLPlugin plugin) {
        super(NAME, plugin, Map.of(HLModule.getModule(IceFireModule.NAME), "Module requires Ice and Fire to be enabled.", HLModule.getModule(SwModule.NAME), "Module requires Spartan Weaponry to be enabled."), Map.of());
        this.plugin = plugin;
    }
    ...
    public void initialize() {
        ...
        HLPlugin.getPlugin().getDebugManager().registerProvider(new cz.hashiri.harshlands.debug.DebugProvider() {
            ...
            @Override public java.util.Collection<String> getSubsystems() { return java.util.List.of("StatusEffects"); }
        });
