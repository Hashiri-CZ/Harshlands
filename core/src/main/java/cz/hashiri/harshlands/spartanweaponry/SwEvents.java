...
public class SwEvents extends ModuleEvents implements Listener {
    ...
    public SwEvents(SwModule module, HLPlugin plugin) {
        super(module, plugin);
        this.plugin = plugin;
    }
    ...
    public void onEntityItemAcquire(EntityItemAcquireEvent event) {
        ...
        if (!(HLItem.isHLItem(item) && (loc == EquipmentSlot.HAND || loc == EquipmentSlot.OFF_HAND)))
            return;
        ...
        HLModule module = HLModule.getModule(HLItem.getModuleNameFromItem(item));
        ...
        switch (type) {
            ...
            case "greatsword", "halberd", "pike" -> {
                ...
                if (!EntityLongAttackTask.hasTask(id) && loc == EquipmentSlot.HAND) {
                    new EntityLongAttackTask(HLModule.getModule(HLItem.getModuleNameFromItem(item)), plugin, entity, item, null).start();
                }
                isThrowing = false;
            }
            case "spear", "lance" -> {
        ...
        if (isThrowing && !EntityPrepareThrowTask.hasTask(id)) {
            new EntityPrepareThrowTask(HLModule.getModule(HLItem.getModuleNameFromItem(item)), plugin, entity, item).start();
        }
    }
    ...
    public void onShootBow(EntityShootBowEvent event) {
        ...
        if (!(shouldEventBeRan(event.getEntity()) && HLItem.isHLItem(bow)))
            return;

        Entity proj = event.getProjectile();
        String moduleName = HLItem.getModuleNameFromItem(bow);

        if (!(moduleName.equals(SwModule.NAME) || moduleName.equals(SfModule.NAME)))
            return;

        FileConfiguration config = moduleName.equals(SwModule.NAME) ? HLModule.getModule(SwModule.NAME).getUserConfig().getConfig() : HLModule.getModule(SfModule.NAME).getUserConfig().getConfig();
        String name = HLItem.getNameFromItem(bow);
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        ...
        if (attacker instanceof AbstractArrow || attacker instanceof Firework) {
            ...
            if (HLItem.isHLItem(itemMainHand)) {
                String moduleName = HLItem.getModuleNameFromItem(itemMainHand);

                if (moduleName.equals(SwModule.NAME) || moduleName.equals(SfModule.NAME)) {
                    FileConfiguration config = HLModule.getModule(moduleName).getUserConfig().getConfig();
    ...
    public void onTarget(EntityTargetEvent event) {
        ...
        switch (type) {
            case "greatsword", "spear", "halberd", "pike", "lance" -> {
                if (!EntityLongAttackTask.hasTask(living.getUniqueId())) {
                    new EntityLongAttackTask(HLModule.getModule(HLItem.getModuleNameFromItem(itemMainHand)), plugin, living, itemMainHand, target).start();
                }
                isThrowing = false;
            }
            case "dagger", "tomahawk", "javelin", "boomerang", "throwing_knife" -> isThrowing = true;
        ...
        if (isThrowing && !EntityPrepareThrowTask.hasTask(living.getUniqueId())) {
            new EntityPrepareThrowTask(HLModule.getModule(HLItem.getModuleNameFromItem(itemMainHand)), plugin, living, itemMainHand).start();
        }
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        HLModule module = HLModule.getModule(HLItem.getModuleNameFromItem(itemMainHand));
