 ...
 */
package cz.hashiri.harshlands.ntp;
...
public class NtpEvents extends ModuleEvents implements Listener {
    ...
    public NtpEvents(NtpModule module, HLPlugin plugin) {
        super(module, plugin);
        this.module = module;
        this.plugin = plugin;
            ...
            if (name.contains("dagger") || name.contains("knife")) {
                if (!itemMainHand.getItemMeta().hasEnchant(Enchantment.SILK_TOUCH)) {
                    if (Utils.matchMaterial(material, config.getConfigurationSection("PlantFiberGathering.BlockDrops").getKeys(false), true)) {
                        ItemStack plantFiber = HLItem.getItem("plant_fiber");
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (action == Action.LEFT_CLICK_BLOCK) {
            ...
            if (config.getBoolean("FlintKnapping.Enabled") && Utils.isItemReal(item) && item.getType() == Material.FLINT && !HLItem.isHLItem(item) && config.getConfigurationSection("FlintKnapping.BlockDrops").getKeys(false).contains(blockMat.toString())) {
            ...
            if (Utils.isItemReal(item) && Utils.isHoldingAxe(player)) {
                if (Tag.LOGS.isTagged(blockMat) || (blockMat.toString().equals("BAMBOO_BLOCK") || blockMat.toString().equals("STRIPPED_BAMBOO_BLOCK"))) {
                    if (HLItem.isHLItem(item) && HLItem.getNameFromItem(item).contains("mattock")) {
    ...
    public void onPrepareCraft(PrepareItemCraftEvent event) {
        ...
        for (int i = 0; i < matrix.length; i++) {
            ...
            if (Utils.isItemReal(item)) {
                ...
                else if (HLItem.isHLItem(item)) {
                    if (HLItem.getNameFromItem(item).equals("clay_tool")) {
                        clayToolIndex = clayToolIndex == -1 ? i : -2;
                    }
                    if (HLItem.getNameFromItem(item).contains("saw")) {
        ...
        if (config.getBoolean("Pottery.DisableFlowerPotRecipe") && clayToolIndex > -1 && clayIndex > -1) {
            ...
            ItemStack clayBrick = HLItem.getItem("clay_brick");
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (Utils.isItemReal(result)) {
            if (Tag.PLANKS.isTagged(result.getType())) {
                ...
                for (ItemStack itemStack : matrix) {
                    if (itemStack != null) {
                        ...
                        if (HLItem.isHLItem(itemStack)) {
                            if (HLItem.getNameFromItem(itemStack).contains("saw")) {
                                tool = itemStack;
                                hasSaw = true;
                            }
                        }
                ...
                for (ItemStack itemStack : matrix) {
                    ...
                    if (HLItem.isHLItem(itemStack) && HLItem.getNameFromItem(itemStack).contains("saw")) {
                        tool = itemStack;
                        hasSaw = true;
                    }
                    if (hasSaw && hasPlanks) {
            ...
            else if (HLItem.isHLItem(result) && HLItem.getNameFromItem(result).equals("clay_brick")) {
                ...
                for (ItemStack itemStack : matrix) {
                    ...
                    if (HLItem.isHLItem(itemStack) && HLItem.getNameFromItem(itemStack).equals("clay_tool")) {
                        tool = itemStack;
                        hasClayTool = true;
                    }
    ...
    public void onBucketInteract(PlayerItemAcquireEvent event) {
        ...
        if (!(HLItem.isHLItem(origItem) && HLItem.getNameFromItem(origItem).contains("ceramic_")))
            return;
        ...
        ItemStack realResult = HLItem.getItem(realResultName);
        new BukkitRunnable() {
            @Override
            public void run() {
    ...
    public void onBucketClick(PlayerItemAcquireEvent event) {
        ...
        if (!(HLItem.isHLItem(item) && HLItem.getNameFromItem(item).equals("ceramic_lava_bucket")))
            return;
