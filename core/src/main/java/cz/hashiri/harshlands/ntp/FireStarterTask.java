...
public class FireStarterTask extends BukkitRunnable {
    ...
    public FireStarterTask(HLPlugin plugin, NtpModule module, Player player, Location loc, Collection<Item> ingredients, boolean isSoulCampfire) {
    ...
    public void run() {
        if (player == null) {
            ...
            if (player.isOnline() && allowedWorlds.contains(player.getWorld().getName()) && loc.getBlock().isPassable() && HLItem.isHoldingItem("fire_starter", player) && validateItems() && player.isSneaking()) {
                ticks++;
                if (ticks > duration) {
