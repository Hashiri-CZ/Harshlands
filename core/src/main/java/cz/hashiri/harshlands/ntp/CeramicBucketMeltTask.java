...
public class CeramicBucketMeltTask extends BukkitRunnable {
    ...
    public CeramicBucketMeltTask(HLPlugin plugin, NtpModule module, Player player) {
        this.player = player;
    ...
    public void run() {
        if (player == null) {
            ...
            if (player.isOnline() && allowedWorlds.contains(player.getWorld().getName()) && HLItem.isHoldingItem("ceramic_lava_bucket", player)) {
                ticks++;
                ...
                if (ticks > duration) {
                    ...
                    if (HLItem.isHoldingItemInMainHand("ceramic_lava_bucket", player))
                        player.getInventory().setItemInMainHand(null);

                    if (HLItem.isHoldingItemInOffHand("ceramic_lava_bucket", player))
