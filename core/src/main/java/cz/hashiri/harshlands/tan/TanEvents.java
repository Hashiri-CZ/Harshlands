...
public class TanEvents extends ModuleEvents implements Listener {
    ...
    public TanEvents(TanModule module, HLPlugin plugin) {
        super(module, plugin);
        this.module = module;
        this.plugin = plugin;
    ...
    public void onDrink(PlayerInteractEvent event) {
        ...
        if (parasites && !ParasiteTask.hasTask(player.getUniqueId()) && !hasStoneSea) {
            new ParasiteTask(module, plugin, HLPlayer.getPlayers().get(player.getUniqueId())).start();
        }
    }
    ...
    public void onConsume(PlayerItemConsumeEvent event) {
        ...
        String name = HLItem.isHLItem(item) ? HLItem.getNameFromItem(item) : item.getType().toString();
    ...
    public void onWorldChange(PlayerChangedWorldEvent event) {
        ...
        if (tempEnabled) {
            ...
            if (HLItem.isHoldingItem("thermometer", player) && !ThermometerTask.hasTask(player.getUniqueId())) {
                new ThermometerTask(plugin, HLPlayer.getPlayers().get(player.getUniqueId())).start();
            }
        }
        if (thirstEnabled && !ThirstCalculateTask.hasTask(player.getUniqueId())) {
            new ThirstCalculateTask(module, plugin, rsvplayer).start();
    ...
    public void onPrepareCraft(PrepareItemCraftEvent event) {
        ...
        if (canteenIndex > -1 && juiceIndex > -1) {
            ...
            if (canteen.getAmount() == 1 && juice.getAmount() == 1) {
                String drinkName = HLItem.isHLItem(juice) ? HLItem.getNameFromItem(juice) : "Unpurified Water";
    ...
    public void onItemAcquire(PlayerItemAcquireEvent event) {
        ...
        if (!(HLItem.isHLItem(item) && HLItem.getNameFromItem(item).equals("thermometer") && (loc == EquipmentSlot.HAND || loc == EquipmentSlot.OFF_HAND)))
            return;

        Player player = event.getPlayer();

        if (HLPlayer.isValidPlayer(player) && player.isOnline() && !ThermometerTask.hasTask(player.getUniqueId()))
            new ThermometerTask(plugin, HLPlayer.getPlayers().get(player.getUniqueId())).start();
    }
    ...
    private ItemStack fillCanteen(ItemStack canteen, String drink, int change) {
        ...
        if (sum < 1) {
            ...
            if (meta.getDisplayName().equals(HLItem.getItem("canteen_empty").getItemMeta().getDisplayName())) {
