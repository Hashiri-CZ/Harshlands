...
public class ThirstCalculateTask extends BukkitRunnable implements HLTask {
    ...
    public ThirstCalculateTask(TanModule module, HLPlugin plugin, HLPlayer player) {
        this.plugin = plugin;
        this.module = module;
    ...
    public void run() {
        ...
        if (conditionsMet(player)) {
            ...
            if (player.hasPermission("harshlands.toughasnails.resistance.thirst.*")) {
                thirstLvl = Math.max(MAXIMUM_THIRST, thirstLvl);
            }
            else {
                if (config.getBoolean("Thirst.Effects.Enabled")) {
                    if (thirstLvl <= config.getDouble("Thirst.Effects.Tiers.Thirsty.Threshold")) {
                        if (!ThirstEffectsTask.hasTask(id)) {
                            new ThirstEffectsTask(module, plugin, this.player).start();
                        }
                    }
                }
            }
