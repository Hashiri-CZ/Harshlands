...
public class DehydrationTask extends BukkitRunnable implements HLTask  {
    ...
    public DehydrationTask(TanModule module, HLPlugin plugin, HLPlayer player) {
        this.plugin = plugin;
    ...
    public void run() {
        ...
        if (conditionsMet(player)) {
            if (!player.hasPermission("harshlands.toughasnails.resistance.thirst.damage")) {
                if (damageEnabled) {
            ...
            if (!player.hasPermission("harshlands.toughasnails.resistance.thirst.potioneffects")) {
