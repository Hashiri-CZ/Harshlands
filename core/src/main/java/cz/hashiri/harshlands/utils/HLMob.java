...
public interface HLMob {
    ...
    default Collection<ItemStack> getConfigurableLoot(@Nullable ItemStack tool) {
        ...
        FileConfiguration config = HLModule.getModule(getParentModule()).getUserConfig().getConfig();
        ...
        if (config.contains("MobDrops." + name())) {
            ...
            for (String itemKey : itemKeys) {
                if (HLItem.isHLItem(itemKey)) {
                    loot.add(Utils.getMobLoot(section.getConfigurationSection(itemKey), HLItem.getItem(itemKey), tool, true));
                }
            }
        }
    ...
    default void applyConfigOptions() {
        ...
        ConfigurationSection section = HLModule.getModule(getParentModule()).getUserConfig().getConfig().getConfigurationSection(name + ".Info");
