...
public class HLAnvilRecipe implements Recipe, HLRecipe {
    ...
    public HLAnvilRecipe(@Nonnull FileConfiguration config, @Nonnull String name) {
        this.name = name;

        this.result = HLRecipe.getResult(config, name);
        leftIng =  HLRecipe.getItem(config, name + ".LeftIngredient.Item", name + ".LeftIngredient.Amount");
        rightIng = HLRecipe.getItem(config, name + ".RightIngredient.Item", name + ".RightIngredient.Amount");
